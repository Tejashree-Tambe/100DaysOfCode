from tkinter import *
import math

# CONSTANTS
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 0.2
SHORT_BREAK_MIN = 0.1
LONG_BREAK_MIN = 0.3
reps = 0
timer = None

# TIMER RESET


def reset_timer():
    window.after_cancel(timer)
    timer_title.config(text="Timer")
    canvas.itemconfig(timer_text, text="00:00")
    work_sessions_done.config(text="")
    global reps
    reps = 0


# TIMER MECHANISM


def start_timer():
    global reps
    reps += 1

    work_min_secs = WORK_MIN * 60
    short_break_min_secs = SHORT_BREAK_MIN * 60
    long_break_min_secs = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        count_down(long_break_min_secs)
        timer_title.config(text="Break", fg=RED)

    elif reps % 2 == 0:
        count_down(short_break_min_secs)
        timer_title.config(text="Break", fg=PINK)

    else:
        count_down(work_min_secs)
        timer_title.config(text="Work", fg=GREEN)

# COUNTDOWN MECHANISM


def count_down(count):
    count_min = int(math.floor(count / 60))
    count_secs = int(count % 60)

    if count_min < 10:
        count_min = f"0{count_min}"

    if count_secs < 10:
        count_secs = f"0{count_secs}"

    canvas.itemconfig(timer_text, text=f"{count_min}:{count_secs}")

    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)

    else:
        start_timer()
        ticks = ""
        for i in range(math.floor(reps/2)):
            ticks += "✔"
        work_sessions_done.config(text=ticks)


# UI SETUP
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)

timer_title = Label(text="Timer", font=(FONT_NAME, 35, "normal"), bg=YELLOW, fg=GREEN)
timer_title.grid(column=1, row=0)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato_img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=tomato_img)
timer_text = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "normal"))
canvas.grid(column=1, row=1)

start_button = Button(text="Start", width=10, highlightthickness=0, command=start_timer)
start_button.grid(column=0, row=2)

reset_button = Button(text="Reset", width=10, highlightthickness=0, command=reset_timer)
reset_button.grid(column=2, row=2)

work_sessions_done = Label(fg=GREEN, bg=YELLOW)
work_sessions_done.grid(column=1, row=3)

window.mainloop()
